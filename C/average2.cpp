...
int main()
{
    std::string filename("lupivot_cmd_O2");
    std::string command;

    std::array<char, 128> buffer;
    std::string result; // Stdout
    std::string outfile = "data_" + filename + ".csv";
    ...
    {
        std::cout << "Start testing size " << size << std::endl;
        // data << size << ",";
        for (int i = 0; i < TIME; i++)
        {
            command = "./" + filename + " --size " + std::to_string(size) + ">>" + outfile;
    ...
    command = "python3 tomktable2.py -f " + outfile;
    pipe = popen(command.c_str(), "r");
    if (!pipe)
    {
        std::cerr << "Couldn't start command." << std::endl;
