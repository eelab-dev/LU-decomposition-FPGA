...
int read_sparse(char *filename, Mtx *mtx)
{
    int ret_code;
    MM_typecode matcode;
    FILE *f;
    ...
    mtx->n = M;
    mtx->nz = nz;
    mtx->Ap = (int *)malloc((M + 1) * sizeof(int));
    mtx->Ai = (int *)malloc(nz * sizeof(int));
    mtx->Ax = (double *)malloc(nz * sizeof(double));

    int *temp;
    double nz2 = 0;
    temp = (int *)calloc(nz, sizeof(int));
