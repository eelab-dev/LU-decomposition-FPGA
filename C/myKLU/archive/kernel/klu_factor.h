...
void factor2(
    ...
    int Ai[], /* size nz, row indices */
    double Ax[],
    int n, int nblocks, int nzoff,
    int *P, int *Q, int *R, double *Lnz, int *Pnum, int *Offp, int *Offi, int *Lip, int *Uip, int *Llen, int *Ulen,
    double *Offx, double *Udiag, double *Rs, int *lusize_sum,
    double *LUbx,
    int *Pinv,
    double *Xwork,
    int *LUsize,
    int *lnz, int *unz, int *max_lnz_block, int *max_unz_block,
    KLU_common *Common)
{
    int k1, k2, nk, oldcol, pend, oldrow, newrow, poff;
    double s;
    ...
    *max_unz_block = 1;
    ...
    if (Common->scale >= 0)
    {
        ...
        KLU_scale(Common->scale, n, Ap, Ai, (double *)Ax, Rs, Pnum, Common);
    ...
    for (int block = 0; block < nblocks; block++)
    {
        ...
        else
        {
            ...
            LUsize[block] = KLU_kernel(nk, Ap, Ai, Ax, Q, lusize, pinv, Pblock, &LUbx[*lusize_sum], Udiag + k1, Llen + k1, Ulen + k1, Lip + k1, Uip + k1, &lnz_block, &unz_block, Xwork, Stack, Flag, Ap_pos, Lpend, k1, Pinv, Rs, Offp, Offi, Offx, Common);
