...
int main(void)
{
    ...
    // int P[] = {0, 3, 2, 4, 1}, Q[] = {0, 3, 2, 4, 1}, R[] = {0, 5, 0, 0, 0}, lusize = 46, k1 = 0, PSinv[] = {0, 4, 2, 1, 3}, Offp[] = {0, 0, 0, 0, 4}, Offi[] = {0, 0, 0, 0, 4}, lnz, unz, nblocks = 1, nzoff = 0, maxblock = 5;
    int P[] = {2, 0, 1, 4, 3}, Q[] = {3, 0, 4, 1, 2}, R[] = {0, 1, 4, 5, 2}, lusize = 46, k1 = 0, PSinv[] = {0, 4, 2, 1, 3}, Offp[] = {0, 0, 0, 0, 4}, Offi[] = {0, 0, 0, 0, 4}, lnz, unz, nblocks = 3, nzoff = 4, maxblock = 3;
    int nzoff1 = nzoff + 1, n1 = n + 1;
    double Rs[] = {3, 6, 3, 1, 4}, Offx[] = {0, 0, 0, 0, 0};

    // int Pinv[5] = {0}, Llen[5] = {0}, Ulen[5], Lip[5] = {0}, Uip[5] = {0}, Stack[5] = {0}, Flag[5] = {0}, Ap_pos[5] = {0}, Lpend[5] = {0};
    // double Udiag[5], X[5], *LU;
    int Pinv[n], Llen[n], Ulen[n], Lip[n], Uip[n], Stack[n], Flag[n], Ap_pos[n], Lpend[n];
    double Udiag[n], X[n], *LU;
    LU = malloc(lusize * sizeof(double));

    // double Lnz[] = {10, 0, 0, 0, 0};
    double Lnz[] = {1, 6, 1, 0, 0};
    Symbolic.n = n;
    Symbolic.P = P;
    Symbolic.Q = Q;
    Symbolic.R = R;
    ...
    Numeric.Llen = Llen;
    Numeric.Ulen = Ulen;
    Numeric.LUsize = malloc(nblocks * sizeof(size_t));
    ...
    printf("n=%d, lusize=%d, lnz=%d, unz=%d, k1=%d\n", n, Numeric.LUsize[0], lnz, unz, k1);
    for (int i = 0; i < n; i++)
    {
        printf("Q[%d]=%ld,Pinv[%d]=%ld,P[%d]=%ld,Udiag[%d]=%lf,Llen[%d]=%ld,Ulen[%d]=%ld,Lip[%d]=%ld,Uip[%d]=%ld,X[%d]=%lf,Stack[%d]=%ld,Flag[%d]=%ld,Lpend[%d]=%ld,Ap_pos[%d]=%ld,Psinv[%d]=%ld,Rs[%d]=%lf,Offp[%d]=%ld,Offi[%d]=%ld,Offx[%d]=%lf\n", i, Q[i], i, Numeric.Pinv[i], i, P[i], i, Udiag[i], i, Llen[i], i, Ulen[i], i, Lip[i], i, Uip[i], i, X[i], i, Stack[i], i, Flag[i], i, Lpend[i], i, Ap_pos[i], i, PSinv[i], i, Rs[i], i, Offp[i], i, Offi[i], i, Offx[i]);
    }
